// 3DRacer.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
private:
	...
	float fSpeed = 1.5f;			// Current player speed
...
protected:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		for (int x = 0; x < ScreenWidth(); x++)
		{
			int nHillHeight = (int)(fabs(sinf(x * 0.05f + fTrackCurvature) * 16.0f));
		...
		for (int y = 0; y < ScreenHeight() / 2; y++)
			for (int x = 0; x < ScreenWidth(); x++)
			{
				...
				int nGrassColor = sinf(20.0f *  powf(1.0f - fPerspective, 3) + fDistance * 0.1f) > 0.0f ? FG_BLUE : FG_DARK_BLUE;
				int nClipColor = sinf(80.0f *  powf(1.0f - fPerspective, 2) + fDistance) > 0.0f ? FG_MAGENTA : FG_WHITE;
